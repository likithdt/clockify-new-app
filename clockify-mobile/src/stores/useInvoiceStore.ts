...
interface InvoiceState {
    ...
    markInvoiceStatus: (id: string, status: "Sent" | "Overdue" | "Paid" | "Draft") => void;
...
export const useInvoiceStore = create<InvoiceState>((set) => ({
    ...
    markInvoiceStatus: (id, status) => {
        set((state) => ({
            invoices: state.invoices.map((inv) => {
                if (inv.id === id) {
                    const balance = status === "Paid" ? 0 : inv.amount;
                    return { ...inv, status, balance };
                }
                return inv;
            }),
            toastMessage: `Invoice status changed to ${status}`,
        }));
    },
    ...
    duplicateInvoice: (id) => {
        set((state) => {
            ...
            return {
                invoices: [cloned, ...state.invoices],
                toastMessage: `Created copy of ${target.invoiceNumber}`,
            };
        });
    },
