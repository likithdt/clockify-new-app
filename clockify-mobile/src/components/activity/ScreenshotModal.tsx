...
export function ScreenshotModal({
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-xs p-2 sm:p-4 select-none animate-in fade-in duration-150">
            <div className="bg-white rounded-2xl shadow-2xl border border-[#e2e8f0] w-full max-w-lg max-h-[92vh] flex flex-col overflow-hidden">
                {/* Modal Header */}
                <div className="px-3.5 py-2.5 border-b border-[#e2e8f0] flex items-center justify-between bg-[#f8fafc]">
                    <div className="flex items-center gap-2 min-w-0 pr-2">
                        <div className="w-6 h-6 rounded-full bg-[#03a9f4] text-white flex items-center justify-center font-bold text-xs shrink-0">
                            <Laptop className="w-3.5 h-3.5" />
                        </div>
                        <div className="truncate">
                            <h3 className="text-xs font-bold text-[#1e293b] flex items-center gap-1 truncate">
                                <span>{screenshot.appName}</span>
                                <span className="text-[10px] font-normal text-[#64748b] truncate">· {screenshot.windowTitle}</span>
                            </h3>
                            <div className="flex items-center gap-1.5 text-[10px] text-[#64748b] truncate">
                                <span className="truncate">{screenshot.memberName}</span>
                                <span>•</span>
                                <span className="font-mono">{screenshot.timeFormatted}</span>
                                <span>•</span>
                                <span
                                    className="font-semibold px-1 py-0.2 rounded text-[9px] shrink-0"
                                    style={{
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                        <span className="px-1.5 py-0.2 bg-[#ecfdf5] text-[#047857] text-[10px] font-mono font-bold rounded">
                            {screenshot.activityPercent}%
                        </span>
                        <button
                            type="button"
                            onClick={() => {
                            ...
                            className="p-1 text-[#ef4444] hover:bg-red-50 rounded transition"
                        ...
                        <button
                            type="button"
                            onClick={onClose}
                            className="p-1 text-[#64748b] hover:text-[#1e293b] hover:bg-[#f1f5f9] rounded transition"
                        >
                            <X className="w-4 h-4" />
                        </button>
                    </div>
                ...
                <div className="px-3.5 py-2.5 border-t border-[#e2e8f0] flex items-center justify-between bg-white gap-2">
                    <button
                        type="button"
                        onClick={onPrevious}
                        className="px-2.5 py-1.5 border border-[#cbd5e1] hover:bg-[#f8fafc] text-[#475569] rounded-lg text-xs font-medium flex items-center gap-1 transition cursor-pointer shrink-0"
                    >
                        <ChevronLeft className="w-3.5 h-3.5" />
                        <span>Prev</span>
                    </button>

                    <span className="text-[11px] text-[#64748b] truncate text-center">
                        Interval: 5m
                    </span>
